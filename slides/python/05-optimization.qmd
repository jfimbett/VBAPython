...
px = pd.DataFrame(data).sort_values(['symbol', 'date'])
...
\begin{aligned}
\min_{x} \quad & c^\top x \\
  	ext{s.t.}\quad & A_{\mathrm{ub}} x \le b_{\mathrm{ub}}, \\
...
\begin{aligned}
\min_{w} \quad & w^\top \Sigma \, w \\
  	ext{s.t.}\quad & \mathbf{1}^\top w = 1,\; w \ge 0, \\
& \mu^\top w \ge r_{\min}, \\
& S w \le \text{cap}
\end{aligned}
$$
...
Let \(\mathrm{RC}_i = w_i\,(\Sigma w)_i\). Then
...
$$
\begin{aligned}
\min_{w,\,t} \quad & t - \lambda\, \mu^\top w \\
  	ext{s.t.}\quad & \mathbf{1}^\top w = 1,\; w \ge 0, \\
& \|U w\|_2 \le \sqrt{t} + \rho
\end{aligned}
$$
