...
- **Never trust user input**
  ...
  - Use libraries (WTForms, Marshmallow, Pydantic…) instead of doing everything by hand
- **Escaping & XSS**
  - Jinja2 auto-escapes by default → do not mark data as safe unless you are sure
- **CSRF**
  - Protect POST forms (Flask-WTF / your own CSRF tokens)
- **Secrets**
  - Store in **env vars** or secret managers, not in Git
  - `SECRET_KEY`, DB passwords, API keys, OAuth tokens…
- **HTTPS**
...
- Why CSRF?
  - Attacker tricks user’s browser into sending a POST to your app
...
- **Code & dependencies**
  ...
  - `Procfile` (Heroku-style) → `web: gunicorn app:app`
- **Configuration**
...
- **Observability**
  - Logging, error tracking (Sentry, Rollbar, …)
...
- Why blueprints?
  - Modularize routes by domain: `auth`, `admin`, `api`, `blog`, …
...
- **Timeouts**
  - Long computations → use background jobs / Celery / caching
- **Static files**
